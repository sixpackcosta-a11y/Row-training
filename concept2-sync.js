...
function plannedMetrics(session){
  const text=`${session.title||''}\n${session.content||''}`.replace(/,/g,'.');
  const work=text.match(/(\d+)\s*[x×]\s*(\d+(?:\.\d+)?)\s*(?:min(?:utos?)?|')/i);
  const recovery=text.match(/(?:rec(?:uperaci[oó]n)?|descanso)\s*[:·-]?\s*(\d+(?:\.\d+)?)\s*(?:min(?:utos?)?|')/i);
  const spm=text.match(/@\s*(\d{2})\b|\b(\d{2})\s*(?:ppm|spm)\b/i);
  const distance=text.match(/\b(\d{3,5})\s*m(?:etros?)?\b/i);
  const repeats=work?Number(work[1]):0,workMinutes=work?Number(work[2]):0,recoveryMinutes=recovery?Number(recovery[1]):0;
  return {expected_distance_m:distance?Number(distance[1]):null,expected_duration_seconds:work?(repeats*workMinutes+Math.max(0,repeats-1)*recoveryMinutes)*60:null,expected_spm:spm?Number(spm[1]||spm[2]):null};
}
function scoreResult(result,intents){
  let best=null;
  for(const i of intents){
    let score=0;
    // La fecha solo sitúa el resultado cerca de una sesión: nunca basta para asignarlo.
...
async function plannedErgoIntents(userId,results){
  ...
  if(!sr.ok)return [];
  return (await sr.json()).map(x=>({...plannedMetrics(x),
