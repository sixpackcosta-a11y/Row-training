...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    const user=await auth.json(),kind=String(req.body?.kind||''),sourceId=req.body?.source_id,sessionId=req.body?.training_session_id;
    ...
    const membership=await rest(`${supabaseUrl}/rest/v1/rower_team_memberships?user_id=eq.${user.id}&team_code=eq.${encodeURIComponent(session.team_code)}&is_rower=eq.true&select=user_id`,{key:serviceKey});
    let belongs=!!membership?.length;
    if(!belongs){const profile=await rest(`${supabaseUrl}/rest/v1/profiles?user_id=eq.${user.id}&team_code=eq.${encodeURIComponent(session.team_code)}&select=user_id`,{key:serviceKey});belongs=!!profile?.length}
    ...
    if(kind==='concept2'){
      ...
      const rows=await rest(`${supabaseUrl}/rest/v1/concept2_results?id=eq.${sourceId}&user_id=eq.${user.id}&select=id`,{key:serviceKey});
      if(!rows?.length)return res.status(404).json({error:'result_not_found'});
      await rest(`${supabaseUrl}/rest/v1/concept2_results?id=eq.${sourceId}&user_id=eq.${user.id}`,{method:'PATCH',key:serviceKey,prefer:'return=minimal',body:{training_session_id:Number(session.id),matched_session_code:session.title,match_status:'matched',match_confidence:100,updated_at:new Date().toISOString()}});
    }else{
      const rows=await rest(`${supabaseUrl}/rest/v1/workout_logs?id=eq.${sourceId}&user_id=eq.${user.id}&select=id,session_type`,{key:serviceKey});
      ...
      await rest(`${supabaseUrl}/rest/v1/workout_logs?id=eq.${sourceId}&user_id=eq.${user.id}`,{method:'PATCH',key:serviceKey,prefer:'return=minimal',body:{training_session_id:Number(session.id),session_code:session.title,session_date:session.session_date}});
    }
